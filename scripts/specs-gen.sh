...
if [[ "$AWS_LAMBDA_FUNCTION_NAME" != "" && "$UP_STAGE" != "" && "$UP_STAGE" != "development" ]]; then
    HOST=$(aws ssm get-parameters --name "/$AWS_LAMBDA_FUNCTION_NAME/$UP_STAGE/host" --with-decryption | jq -r '.Parameters[0].Value')
fi
